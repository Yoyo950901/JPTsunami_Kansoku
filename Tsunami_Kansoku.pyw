...
try:
    for i in item:
        try:
            ...
            name = i["Station"]["Name"]
            try:
                maxheitime = i["Station"]["MaxHeight"]["DateTime"]
                maxheitimeh = maxheitime[11:13]
                if int(maxheitimeh) > 12:
                    maxheitimeh = int(maxheitimeh) - 12
                    ampm = "午後"
                ...
                maxheitime = ""
            try:
                height = i["Station"]["MaxHeight"]["jmx_eb:TsunamiHeight"]["@description"]
            except:
                height = i["Station"]["MaxHeight"]["Condition"]
            try:
                if i["Station"]["MaxHeight"]["jmx_eb:TsunamiHeight"]["@condition"] == "上昇中":
                    rising = "(上昇中)"
            except:
                rising = ""
            print(f"{name}　{ampm}{maxheitime}{height}{rising}")
    ...
    try:
        ...
        ampm = ""
        try:
            maxheitime = item["Station"]["MaxHeight"]["DateTime"]
            maxheitimeh = maxheitime[11:13]
            if int(maxheitimeh) > 12:
                maxheitimeh = int(maxheitimeh) - 12
                ampm = "午後"
            ...
            maxheitime = ""
        try:
            height = item["Station"]["MaxHeight"]["jmx_eb:TsunamiHeight"]["@description"]
        except:
            height = item["Station"]["MaxHeight"]["Condition"]
        try:
            if item["Station"]["MaxHeight"]["jmx_eb:TsunamiHeight"]["@condition"] == "上昇中":
                rising = "(上昇中)"
        except:
            rising = ""
        print(f"{name}　{ampm}{maxheitime}{height}{rising}")